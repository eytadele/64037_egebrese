...
# Purpose of regularization in training predictive models: 
...
# Role of loss function in predictive model and some of the common loss functions used for regression and classification models: 
...
# Role of the lambda parameter in regularized linear models such as Lasso or Ridge regression models:
...
# Build a Lasso regression model to predict Sales based on all other attributes ("Price", "Advertising", "Population", "Age", "Income" and "Education").  What is the best value of lambda for such a lasso model?
...
# How many attributes remain in the model if lambda is set to 0.01? How that number changes if lambda is increased to 0.1? Do you expect more variables to stay in the model (i.e., to have non-zero coefficients) as we increase lambda? 
```{r warning=FALSE, error=FALSE, message=FALSE}
...
# Build an elastic-net model with alpha set to 0.6. What is the best value of lambda for such a model?
